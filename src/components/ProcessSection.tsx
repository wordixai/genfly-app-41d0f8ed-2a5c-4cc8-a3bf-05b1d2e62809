    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="w-full max-w-56 pt-4 md:px-2 flex flex-col gap-3">
        <div className="flex items-center gap-2">
          {icon}
          <MockDataLine width="w-6" />
          <MockDataLine width="w-12" />
        </div>
        {mockData}
      </div>
    </div>
...
function MockDataLine({ width }: { width: string }) {
  return (
    <div className={`h-4 ${width} flex flex-col justify-center`}>
    ...
    <div className="flex flex-row gap-2 max-w-56">
      {authProviders.map((provider, i) => (
        <div key={i} className="w-10 h-10 bg-muted rounded-lg flex items-center justify-center">
          ...
          {provider.name === "MetaMask" && <MetaMaskIcon />}
        </div>
      ))}
